...
function getRequestBodySchemaName(requestBody) {
  return (
    requestBody && getSchemaName(requestBody.content['application/json'].schema)
  );
}
...
function getResponseSchemaName(responses) {
  ...
  return (
    response.content &&
    getSchemaName(response.content['application/json'].schema)
  );
}
...
(async () => {
  ...
  await fs.writeFile(
    __dirname + '/../src/api-generated/types.d.ts',
    generatedMessage + typesFile,
  );
    ...
    if (operation.get) {
      ...
      tags[tag].operations[operation.get.operationId] = {
        ...
        parameters:
          ...
          operation.get.parameters.map(({ name, required }) => ({
            name,
            required,
          })),
        response,
      ...
      tags[tag].operations[operation.post.operationId] = {
        ...
        body,
        response,
        recaptcha: (operation.post.description || '').startsWith(
          'Recaptcha required.',
        ),
      };
  ...
  for (const moduleName in tags) {
      ...
      if (operation.type === 'post') {
        const bodyType = operation.body ? `ApiTypes.${operation.body}` : 'void',
          responseType = operation.response
            ? `ApiTypes.${operation.response}`
            : 'void';
        code += `export const ${functionName} = createPostApi<${bodyType}, ${responseType}>(${JSON.stringify(
          path,
        )}, ${operation.recaptcha});\n`;
      } else {
    ...
    await fs.writeFile(
      __dirname +
        `/../src/api-generated/modules/${normalizeModuleName(
          moduleName,
          true,
        )}.ts`,
      code,
  ...
  code += generatedMessage;
  for (const moduleName in tags) {
    code += `import * as Imported${normalizeModuleName(
      moduleName,
    )}Api from "./modules/${normalizeModuleName(moduleName, true)}";\n`;
  }
  code += '\n';
  for (const moduleName in tags) {
    const importedModule = normalizeModuleName(moduleName);
    const exportModule =
      importedModule.charAt(0).toLowerCase() + importedModule.slice(1);
